...
\begin{algorithm}[htb!] \def\baselinestretch{1} \scriptsize\raggedright
	\begin{algorithmic}[1] 
		...
		\SPACE 
		\color{red}\UPON{$f+1$ $\li{\Prevote,h_p, round_p,\nil}$ \With\ $step_p \leq \prevote$ for the first time}
		...
		\SPACE 
		\color{black}\UPON{$\li{\Proposal,h_p,round_p, v, *}$ \From\ $\coord(h_p,round_p)$
			\textbf{AND} $2f+1$ $\li{\Prevote,h_p, round_p,id(v)}$  \With\ $valid(v) \wedge step_p \le \prevote$ for the first time}
