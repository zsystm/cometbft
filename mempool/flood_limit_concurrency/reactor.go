package flood_limit_concurrency
...
func NewReactor(config *cfg.MempoolConfig, mempool *mempool.CListMempool, logger log.Logger) *Reactor {
	...
	memR.sem = semaphore.NewWeighted(int64(memR.config.FloodLimitConcurrency))
...
func (memR *Reactor) broadcastTxRoutine(peer p2p.Peer) {
	...
	for {
		if err := memR.looper(ctx, peer, &next); err != nil {
			return
		}
	}
}

func (memR *Reactor) looper(ctx context.Context, peer p2p.Peer, next **clist.CElement) error {
	//Acquire semaphore and defer the release
	if memR.config.FloodLimitConcurrency > 0 {

		if err := memR.sem.Acquire(ctx, 1); err != nil {
