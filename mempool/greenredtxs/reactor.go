...
type Reactor struct {
	...
	// Two set of tx keys to mark when a tx was already sent and when it should not be send anymore.
	// TODO: these two maps could be merged into one.
	greenTxs sync.Map // txs sent at least once
	redTxs   sync.Map // txs marked to not send anymore
}

// NewReactor returns a new Reactor with the given config and mempool.
func NewReactor(config *cfg.MempoolConfig, mempool *mempool.CListMempool, logger log.Logger) *Reactor {
	memR := &Reactor{
		config:    config,
		mempool:   mempool,
	...
	memR.mempool.SetTxRemovedCallback(func(txKey types.TxKey) {
		...
		memR.greenTxs.Delete(txKey)
		memR.redTxs.Delete(txKey)
	})
	...
	case *protomem.Txs:
		...
		for _, txBytes := range protoTxs {
			...
			if errors.Is(err, mempool.ErrTxInCache) {
				...
				// if tx was sent at least once (green), do not send it anymore (mark it red)
				if _, ok := memR.greenTxs.Load(tx.Key()); ok {
					memR.Logger.Info("It's green 🟢, mark red 🚩", "tx", tx.Key())
					memR.redTxs.Store(tx.Key(), struct{}{})
				}
			} else if err != nil {
...
func (memR *Reactor) broadcastTxRoutine(peer p2p.Peer) {
	...
	for {
		...
		txKey := memTx.GetTx().Key()

		// if tx is marked red, do not send it
		if _, ok := memR.redTxs.Load(txKey); ok {
			memR.Logger.Info("It's red 🛑; do not send", "tx", txKey)
			continue
		}
		...
		if !memR.isSender(txKey, peer.ID()) {
			...
			if success {
				// tx was sent at least once: mark green
				memR.Logger.Info("Mark green 🟢", "tx", txKey)
				memR.greenTxs.Store(txKey, struct{}{})
			} else {
				time.Sleep(mempool.PeerCatchupSleepIntervalMS * time.Millisecond)
				continue
			}
