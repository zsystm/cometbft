	...
	for {
		...
		if !memR.isSender(txKey, peer.ID()) {
			...
			if !success {
				time.Sleep(mempool.PeerCatchupSleepIntervalMS * time.Millisecond)
				continue
			} else if memR.config.FloodRandomSleep > 0 {
				var sleepBase = memR.config.FloodRandomSleep
				var sleepVariation = time.Duration(rand.Intn(int(memR.config.FloodRandomSleep / 10)))
				var sleepTotal = sleepBase + sleepVariation
				time.Sleep(sleepTotal * time.Millisecond)
			}
